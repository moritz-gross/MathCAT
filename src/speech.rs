...
fn intent_rules<'m>(rules: &'static std::thread::LocalKey<RefCell<SpeechRules>>, doc: Document<'m>, mathml: Element, nav_node_id: &'m str) -> Result<Element<'m>> {
    rules.with(|rules| {
        ...
        let intent =  rules_with_context.match_pattern::<Element<'m>>(mathml)
                    .with_context(|| "Pattern match/replacement failure!")?;
        ...
        match key {
            ...
            "x" => {
                return Ok( Replacement::XPath( MyXPath::build(value)
                    .with_context(|| "while trying to evaluate value of 'x:'")? ) );
            },
            ...
            "translate" => {
                return Ok( Replacement::Translate( TranslateExpression::build(value)
                    .with_context(|| "while trying to evaluate value of 'speak:'")? ) );
            },
            _ => {
...
impl InsertChildren {
    fn build(insert: &Yaml) -> Result<Box<InsertChildren>> {
        ...
        return Ok( Box::new( InsertChildren {
            ...
            replacements: ReplacementArray::build(replace).with_context(|| "'replace:'")?,
        } ) );
    }
...
impl Intent {
    fn build(yaml_dict: &Yaml) -> Result<Box<Intent>> {
        ...
        return Ok( Box::new( Intent {
            name: if name.is_badvalue() {None} else {Some(as_str_checked(name).with_context(|| "'name'")?.to_string())},
            xpath: if xpath_name.is_badvalue() {None} else {Some(MyXPath::build(xpath_name).with_context(|| "'intent'")?)},
            attrs: if attrs.is_badvalue() {"".to_string()} else {as_str_checked(attrs).with_context(|| "'attrs'")?.to_string()},
            children: ReplacementArray::build(replace).with_context(|| "'children:'")?,
        } ) );
    }
        
    fn replace<'c, 's:'c, 'm: 'c, T:TreeOrString<'c, 'm, T>>(&self, rules_with_context: &mut SpeechRulesWithContext<'c, 's,'m>, mathml: Element<'c>) -> Result<T> {
        let result = self.children.replace::<Element<'m>>(rules_with_context, mathml)
                    .with_context(||"replacing inside 'intent'")?;
        ...
        if !self.attrs.is_empty() {
            ...
            for cap in ATTR_NAME_VALUE.captures_iter(&self.attrs) {
                ...
                let value_as_xpath = MyXPath::new(matched_value.to_string()).with_context(||"attr value inside 'intent'")?;
                let value = value_as_xpath.evaluate(rules_with_context.get_context(), result)
                        .with_context(||"attr xpath evaluation value inside 'intent'")?;
...
impl With {
    fn build(vars_replacements: &Yaml) -> Result<Box<With>> {
        ...
        return Ok( Box::new( With {
            variables: VariableDefinitions::build(var_defs).with_context(|| "'variables'")?,
            replacements: ReplacementArray::build(replace).with_context(|| "'replace:'")?,
        } ) );
    }
    ...
    fn replace<'c, 's:'c, 'm: 'c, T:TreeOrString<'c, 'm, T>>(&self, rules_with_context: &mut SpeechRulesWithContext<'c, 's,'m>, mathml: Element<'c>) -> Result<T> {
        ...
        let result = self.replacements.replace(rules_with_context, mathml)
                    .with_context(||"replacing inside 'with'")?;
...
impl SetVariables {
    fn build(vars: &Yaml) -> Result<Box<SetVariables>> {
        ...
        return Ok( Box::new( SetVariables {
            variables: VariableDefinitions::build(vars).with_context(|| "'set_variables'")?
        } ) );
    }
...
impl TranslateExpression {
    fn build(vars: &Yaml) -> Result<TranslateExpression> {
        ...
        return Ok( TranslateExpression { id: MyXPath::build(vars).with_context(|| "'translate'")? } );
    }
    ...
    fn replace<'c, 's:'c, 'm:'c, T:TreeOrString<'c, 'm, T>>(&self, rules_with_context: &mut SpeechRulesWithContext<'c, 's,'m>, mathml: Element<'c>) -> Result<T> {
        if self.id.rc.string.contains('@') {
            ...
            return T::from_string(
                self.id.replace(rules_with_context, mathml).with_context(||"'translate'")?,
    ...
    fn is_true(&self, context: &XPathContext, mathml: Element) -> Result<bool> {
        return match self.condition.as_ref() {
            ...
            Some(condition) => condition.is_true(context, mathml)
                                .with_context(|| "Failure in conditional test"),
        }
    }
}
        ...
        if defs.is_array() {
            ...
            for def in defs {
                let variable_def = VariableDefinition::build(def)
                        .with_context(|| "definition of 'variables'")?;
